	...
	<form action="" method="post">
		...
		<table align="center" border="1" cellspacing="14" cellpadding="12" style="color: black" >
				...
				<tr align="center">
					...
					<td><input type="date" name="edate" required min="<?php echo date('Y-m-d'); ?>"></td>
			       
				    <tr align="center">
			    	<td style="color: red">   QTY</td>
			        <td> <input type="number" name="qty" value="" min=1 max=150 required></td>
			    </tr>
			...
			if(isset($_POST['sb']))
			{
				...
				include "connect.php";
				mysqli_query($con,"insert into addcart(p_id,u_id,price,edate,qty,total ) values('$pid','$uid','$price','$edate','$qty','$total')") or die(mysqli_error($con));
				echo "<script>alert('Your data Is Add Inside Cart')</script>";
				header('URL = idorder.php');
